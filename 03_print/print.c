...
void 
__put_char(char *p, int num)
{
    while(*p && num -- )
    {
        *(volatile unsigned int *)0x601F1000 = *p ++;
    }
}
...
char *
number(char *str, int num, int base, unsigned int flags)
{
    int i = 0;
    int sign = 0;
...
int
format_decode(const char *fmt, unsigned int *flags)
{
